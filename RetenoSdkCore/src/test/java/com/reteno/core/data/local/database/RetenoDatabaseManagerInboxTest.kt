...
class RetenoDatabaseManagerInboxTest : BaseRobolectricTest() {
    ...
    fun givenInboxesAvailableInDatabase_whenGetAppInbox_thenInboxesReturned() {
        ...
        assertEquals(inbox1, interactions?.get(0))
        assertEquals(inbox2, interactions?.get(1))
    }
    ...
    fun givenInboxCorruptedInDatabaseAndRowIdDetected_whenGetAppInbox_thenCorruptedRowRemoved() {
        ...
        verify(exactly = 1) {
            database.delete(
                DbSchema.AppInboxSchema.TABLE_NAME_APP_INBOX,
    ...
    fun givenInboxCountEmpty_whenGetAppInboxCount_thenZeroReturned() {
        val recordsCount = 0L
        every { database.getRowCount(DbSchema.AppInboxSchema.TABLE_NAME_APP_INBOX) } returns recordsCount
    ...
    fun whenDeleteInboxesByTime_thenInboxesDeleted() {
        ...
        val whereClauseExpected =
            "${DbSchema.AppInboxSchema.COLUMN_APP_INBOX_TIME} < '$outdatedTime'"
    ...
    fun whenDeleteAllInboxes_thenInboxesDeleteAllInboxes_thenInboxesDeleted() {
        ...
        verify(exactly = 1) {
            database.delete(DbSchema.AppInboxSchema.TABLE_NAME_APP_INBOX)
        }
    }
    ...
    private fun mockColumnIndexes() {
        ...
        every { cursor.getColumnIndex(DbSchema.AppInboxSchema.COLUMN_APP_INBOX_TIME) } returns COLUMN_INDEX_INBOX_TIME
        every { cursor.getColumnIndex(DbSchema.AppInboxSchema.COLUMN_APP_INBOX_DEVICE_ID) } returns COLUMN_INDEX_INBOX_DEVICE_ID
    }
    ...
    private fun mockDatabaseQuery() {
        every {
            database.query(
                table = DbSchema.AppInboxSchema.TABLE_NAME_APP_INBOX,
