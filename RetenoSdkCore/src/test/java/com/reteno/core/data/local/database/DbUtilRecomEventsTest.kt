...
class DbUtilRecomEventsTest : BaseRobolectricTest() {
    ...
    fun givenRecomVariantIdProvided_whenPutRecomVariantId_thenContentValuesUpdated() {
        // Given
        val keySet = arrayOf(DbSchema.RecomEventsSchema.COLUMN_RECOM_VARIANT_ID)
        ...
        assertEquals(
            RECOM_VARIANT_ID,
            contentValues.get(DbSchema.RecomEventsSchema.COLUMN_RECOM_VARIANT_ID)
        )
    }
    ...
    fun givenRecomEventsProvided_whenToContentValuesList_thenContentValuesListReturned() {
        ...
        assertEquals(
            RECOM_VARIANT_ID,
            contentValues1.get(DbSchema.RecomEventsSchema.COLUMN_RECOM_VARIANT_ID)
        )
        assertEquals(
            RECOM_EVENT_TYPE_1.toString(),
            contentValues1.get(DbSchema.RecomEventSchema.COLUMN_RECOM_EVENT_TYPE)
        )
        assertEquals(
            RECOM_EVENT_OCCURRED_1.formatToRemote(),
            contentValues1.get(DbSchema.RecomEventSchema.COLUMN_RECOM_EVENT_OCCURRED)
        )
        assertEquals(
            PRODUCT_ID_1,
            contentValues1.get(DbSchema.RecomEventSchema.COLUMN_RECOM_EVENT_PRODUCT_ID)
        )

        assertEquals(
            RECOM_VARIANT_ID,
            contentValues2.get(DbSchema.RecomEventsSchema.COLUMN_RECOM_VARIANT_ID)
        )
        assertEquals(
            RECOM_EVENT_TYPE_2.toString(),
            contentValues2.get(DbSchema.RecomEventSchema.COLUMN_RECOM_EVENT_TYPE)
        )
        assertEquals(
            RECOM_EVENT_OCCURRED_2.formatToRemote(),
            contentValues2.get(DbSchema.RecomEventSchema.COLUMN_RECOM_EVENT_OCCURRED)
        )
        assertEquals(
            PRODUCT_ID_2,
            contentValues2.get(DbSchema.RecomEventSchema.COLUMN_RECOM_EVENT_PRODUCT_ID)
        )
    }
    ...
    private fun mockColumnIndexes() {
        ...
        every { cursor.getColumnIndex(DbSchema.RecomEventSchema.COLUMN_RECOM_EVENT_ROW_ID) } returns COLUMN_INDEX_RECOM_EVENT_ROW_ID
        every { cursor.getColumnIndex(DbSchema.RecomEventSchema.COLUMN_RECOM_EVENT_PRODUCT_ID) } returns COLUMN_INDEX_RECOM_EVENT_PRODUCT_ID
        every { cursor.getColumnIndex(DbSchema.RecomEventSchema.COLUMN_RECOM_EVENT_OCCURRED) } returns COLUMN_INDEX_RECOM_EVENT_OCCURRED
        every { cursor.getColumnIndex(DbSchema.RecomEventSchema.COLUMN_RECOM_EVENT_TYPE) } returns COLUMN_INDEX_RECOM_EVENT_TYPE
    }
