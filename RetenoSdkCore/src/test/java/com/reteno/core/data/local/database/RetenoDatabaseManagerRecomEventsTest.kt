...
class RetenoDatabaseManagerRecomEventsTest : BaseRobolectricTest() {
    ...
    private lateinit var SUT: RetenoDatabaseManagerImpl
    ...
    fun givenValidEventsProvidedRecomVariantIdNotPresentInDatabase_whenInsertEvents_thenEventsIsSavedToDb() {
        ...
        every { database.insert(DbSchema.RecomEventsSchema.TABLE_NAME_RECOM_EVENTS, null, any()) } answers {
        ...
        verify(exactly = 1) {
            database.insert(
                table = eq(DbSchema.RecomEventsSchema.TABLE_NAME_RECOM_EVENTS),
                contentValues = any()
            )
        }
        ...
        verify(exactly = 1) {
            database.insertMultiple(
                table = eq(DbSchema.RecomEventSchema.TABLE_NAME_RECOM_EVENT),
    ...
    fun givenValidRecomEventsProvidedRecomVariantIdPresentInDatabase_whenInsertRecomEvents_thenRecomEventsIsSavedToDb() {
        ...
        verify(exactly = 1) {
            database.query(
                table = DbSchema.RecomEventsSchema.TABLE_NAME_RECOM_EVENTS,
                columns = DbSchema.RecomEventsSchema.getAllColumns(),
                selection = "${DbSchema.RecomEventsSchema.COLUMN_RECOM_VARIANT_ID}=?",
    ...
    fun givenEventsAvailableInDatabase_whenGetEvents_thenEventsReturned() {
        ...
        verify(exactly = 1) {
            database.query(
                table = DbSchema.RecomEventSchema.TABLE_NAME_RECOM_EVENT,
    ...
    fun givenRecomEventsParentNotAvailableInDatabase_whenGetRecomEvents_thenEmptyListReturned() {
        ...
        verify(exactly = 1) {
            database.query(
                table = eq(DbSchema.RecomEventsSchema.TABLE_NAME_RECOM_EVENTS),
                columns = eq(DbSchema.RecomEventsSchema.getAllColumns())
            )
        }
        verify(exactly = 1) { cursor.close() }
    ...
    fun givenRecomEventCorruptedInDatabaseAndRowIdDetected_whenGetRecomEvents_thenCorruptedRowRemoved() {
        ...
        verify(exactly = 1) {
            database.delete(
                table = DbSchema.RecomEventSchema.TABLE_NAME_RECOM_EVENT,
                whereClause = "${DbSchema.RecomEventSchema.COLUMN_RECOM_EVENT_ROW_ID}=?",
    ...
    fun givenRecomEventCountEmpty_whenGetRecomEventsCount_thenZeroReturned() {
        // Given
        val recordsCount = 0L
        every { database.getRowCount(DbSchema.RecomEventSchema.TABLE_NAME_RECOM_EVENT) } returns recordsCount
    ...
    fun given_whenDeleteRecomEventsOldest_thenRecomEventsDeleted() {
        ...
        val order = "ASC"
        val count = 2
        val whereClauseExpected = "${DbSchema.RecomEventSchema.COLUMN_RECOM_EVENT_ROW_ID} " +
                    "in (select ${DbSchema.RecomEventSchema.COLUMN_RECOM_EVENT_ROW_ID} " +
                    "from ${DbSchema.RecomEventSchema.TABLE_NAME_RECOM_EVENT} " +
                    "ORDER BY ${DbSchema.RecomEventSchema.COLUMN_RECOM_EVENT_OCCURRED} $order " +
                    "LIMIT $count)"
        ...
        // Then
        verify(exactly = 1) { database.delete(DbSchema.RecomEventSchema.TABLE_NAME_RECOM_EVENT, whereClauseExpected) }
    }
    ...
    fun whenDeleteRecomEventsByTime_thenRecomEventsDeleted() {
        ...
        val countExpected = 2
        val whereClauseExpected = "${DbSchema.RecomEventSchema.COLUMN_RECOM_EVENT_OCCURRED} < '$outdatedTime'"
    ...
    private fun mockColumnIndexes() {
        ...
        every { cursor.getColumnIndex(DbSchema.RecomEventsSchema.COLUMN_RECOM_VARIANT_ID) } returns COLUMN_INDEX_RECOM_VARIANT_ID

        every { cursorChild.getColumnIndex(DbSchema.RecomEventsSchema.COLUMN_RECOM_VARIANT_ID) } returns COLUMN_INDEX_RECOM_VARIANT_ID
        every { cursorChild.getColumnIndex(DbSchema.RecomEventSchema.COLUMN_RECOM_EVENT_TYPE) } returns COLUMN_INDEX_RECOM_EVENT_TYPE
        every { cursorChild.getColumnIndex(DbSchema.RecomEventSchema.COLUMN_RECOM_EVENT_PRODUCT_ID) } returns COLUMN_INDEX_RECOM_EVENT_PRODUCT_ID
        every { cursorChild.getColumnIndex(DbSchema.RecomEventSchema.COLUMN_RECOM_EVENT_OCCURRED) } returns COLUMN_INDEX_RECOM_EVENT_OCCURRED
        every { cursorChild.getColumnIndex(DbSchema.RecomEventSchema.COLUMN_RECOM_EVENT_ROW_ID) } returns COLUMN_INDEX_RECOM_EVENT_ROW_ID
